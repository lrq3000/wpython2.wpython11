...
static PyTypeObject *expr_const_type;
static PyObject *no_const_singleton, *dummy_1_singleton,
*mutable_const_singleton, *dummy_2_singleton, *dummy_3_singleton,
*dummy_4_singleton, *content_const_singleton, *dummy_5_singleton,
*dummy_6_singleton, *dummy_7_singleton, *dummy_8_singleton, *dummy_9_singleton,
*dummy_10_singleton, *dummy_11_singleton, *pure_const_singleton;
static PyObject* ast2obj_expr_const(expr_const_ty);
static PyTypeObject *no_const_type;
static PyTypeObject *dummy_1_type;
static PyTypeObject *mutable_const_type;
static PyTypeObject *dummy_2_type;
static PyTypeObject *dummy_3_type;
static PyTypeObject *dummy_4_type;
static PyTypeObject *content_const_type;
static PyTypeObject *dummy_5_type;
static PyTypeObject *dummy_6_type;
static PyTypeObject *dummy_7_type;
static PyTypeObject *dummy_8_type;
static PyTypeObject *dummy_9_type;
static PyTypeObject *dummy_10_type;
static PyTypeObject *dummy_11_type;
static PyTypeObject *pure_const_type;
...
static int init_types(void)
{
        ...
        expr_const_type = make_type("expr_const", &AST_type, NULL, 0);
        if (!expr_const_type) return 0;
        if (!add_attributes(expr_const_type, NULL, 0)) return 0;
        no_const_type = make_type("no_const", expr_const_type, NULL, 0);
        if (!no_const_type) return 0;
        no_const_singleton = PyType_GenericNew(no_const_type, NULL, NULL);
        if (!no_const_singleton) return 0;
        dummy_1_type = make_type("dummy_1", expr_const_type, NULL, 0);
        if (!dummy_1_type) return 0;
        dummy_1_singleton = PyType_GenericNew(dummy_1_type, NULL, NULL);
        if (!dummy_1_singleton) return 0;
        mutable_const_type = make_type("mutable_const", expr_const_type, NULL,
                                       0);
        if (!mutable_const_type) return 0;
        mutable_const_singleton = PyType_GenericNew(mutable_const_type, NULL,
                                                    NULL);
        if (!mutable_const_singleton) return 0;
        dummy_2_type = make_type("dummy_2", expr_const_type, NULL, 0);
        if (!dummy_2_type) return 0;
        dummy_2_singleton = PyType_GenericNew(dummy_2_type, NULL, NULL);
        if (!dummy_2_singleton) return 0;
        dummy_3_type = make_type("dummy_3", expr_const_type, NULL, 0);
        if (!dummy_3_type) return 0;
        dummy_3_singleton = PyType_GenericNew(dummy_3_type, NULL, NULL);
        if (!dummy_3_singleton) return 0;
        dummy_4_type = make_type("dummy_4", expr_const_type, NULL, 0);
        if (!dummy_4_type) return 0;
        dummy_4_singleton = PyType_GenericNew(dummy_4_type, NULL, NULL);
        if (!dummy_4_singleton) return 0;
        content_const_type = make_type("content_const", expr_const_type, NULL,
                                       0);
        if (!content_const_type) return 0;
        content_const_singleton = PyType_GenericNew(content_const_type, NULL,
                                                    NULL);
        if (!content_const_singleton) return 0;
        dummy_5_type = make_type("dummy_5", expr_const_type, NULL, 0);
        if (!dummy_5_type) return 0;
        dummy_5_singleton = PyType_GenericNew(dummy_5_type, NULL, NULL);
        if (!dummy_5_singleton) return 0;
        dummy_6_type = make_type("dummy_6", expr_const_type, NULL, 0);
        if (!dummy_6_type) return 0;
        dummy_6_singleton = PyType_GenericNew(dummy_6_type, NULL, NULL);
        if (!dummy_6_singleton) return 0;
        dummy_7_type = make_type("dummy_7", expr_const_type, NULL, 0);
        if (!dummy_7_type) return 0;
        dummy_7_singleton = PyType_GenericNew(dummy_7_type, NULL, NULL);
        if (!dummy_7_singleton) return 0;
        dummy_8_type = make_type("dummy_8", expr_const_type, NULL, 0);
        if (!dummy_8_type) return 0;
        dummy_8_singleton = PyType_GenericNew(dummy_8_type, NULL, NULL);
        if (!dummy_8_singleton) return 0;
        dummy_9_type = make_type("dummy_9", expr_const_type, NULL, 0);
        if (!dummy_9_type) return 0;
        dummy_9_singleton = PyType_GenericNew(dummy_9_type, NULL, NULL);
        if (!dummy_9_singleton) return 0;
        dummy_10_type = make_type("dummy_10", expr_const_type, NULL, 0);
        if (!dummy_10_type) return 0;
        dummy_10_singleton = PyType_GenericNew(dummy_10_type, NULL, NULL);
        if (!dummy_10_singleton) return 0;
        dummy_11_type = make_type("dummy_11", expr_const_type, NULL, 0);
        if (!dummy_11_type) return 0;
        dummy_11_singleton = PyType_GenericNew(dummy_11_type, NULL, NULL);
        if (!dummy_11_singleton) return 0;
        pure_const_type = make_type("pure_const", expr_const_type, NULL, 0);
        if (!pure_const_type) return 0;
        pure_const_singleton = PyType_GenericNew(pure_const_type, NULL, NULL);
        if (!pure_const_singleton) return 0;
...
static int obj2ast_expr_const(PyObject* obj, expr_const_ty* out, PyArena*
                              arena);
...
expr_ty
Const(object c, expr_const_ty constant, int lineno, int col_offset, PyArena
      *arena)
{
        expr_ty p;
        if (!c) {
                PyErr_SetString(PyExc_ValueError,
                                "field c is required for Const");
                return NULL;
        }
        if (!constant) {
                PyErr_SetString(PyExc_ValueError,
                                "field constant is required for Const");
                return NULL;
        }
        p = (expr_ty)PyArena_Malloc(arena, sizeof(*p));
        if (!p)
...
ast2obj_expr(void* _o)
{
        ...
        case Const_kind:
                ...
                if (!result) goto failed;
                value = ast2obj_object(o->v.Const.c);
                if (!value) goto failed;
                if (PyObject_SetAttrString(result, "c", value) == -1)
                        goto failed;
                Py_DECREF(value);
                value = ast2obj_expr_const(o->v.Const.constant);
                if (!value) goto failed;
                if (PyObject_SetAttrString(result, "constant", value) == -1)
                        goto failed;
                Py_DECREF(value);
                break;
        }
...
PyObject* ast2obj_expr_const(expr_const_ty o)
{
        switch(o) {
                case no_const:
                        Py_INCREF(no_const_singleton);
                        return no_const_singleton;
                case dummy_1:
                        Py_INCREF(dummy_1_singleton);
                        return dummy_1_singleton;
                case mutable_const:
                        Py_INCREF(mutable_const_singleton);
                        return mutable_const_singleton;
                case dummy_2:
                        Py_INCREF(dummy_2_singleton);
                        return dummy_2_singleton;
                case dummy_3:
                        Py_INCREF(dummy_3_singleton);
                        return dummy_3_singleton;
                case dummy_4:
                        Py_INCREF(dummy_4_singleton);
                        return dummy_4_singleton;
                case content_const:
                        Py_INCREF(content_const_singleton);
                        return content_const_singleton;
                case dummy_5:
                        Py_INCREF(dummy_5_singleton);
                        return dummy_5_singleton;
                case dummy_6:
                        Py_INCREF(dummy_6_singleton);
                        return dummy_6_singleton;
                case dummy_7:
                        Py_INCREF(dummy_7_singleton);
                        return dummy_7_singleton;
                case dummy_8:
                        Py_INCREF(dummy_8_singleton);
                        return dummy_8_singleton;
                case dummy_9:
                        Py_INCREF(dummy_9_singleton);
                        return dummy_9_singleton;
                case dummy_10:
                        Py_INCREF(dummy_10_singleton);
                        return dummy_10_singleton;
                case dummy_11:
                        Py_INCREF(dummy_11_singleton);
                        return dummy_11_singleton;
                case pure_const:
                        Py_INCREF(pure_const_singleton);
                        return pure_const_singleton;
                default:
                        /* should never happen, but just in case ... */
                        PyErr_Format(PyExc_SystemError, "unknown expr_const found");
                        return NULL;
        }
}
...
obj2ast_expr(PyObject* obj, expr_ty* out, PyArena* arena)
{
        ...
        if (PyObject_IsInstance(obj, (PyObject*)Const_type)) {
                ...
                if (PyObject_HasAttrString(obj, "c")) {
                        int res;
                        tmp = PyObject_GetAttrString(obj, "c");
                        if (tmp == NULL) goto failed;
                        res = obj2ast_constant(tmp, &c, arena);
                        if (res != 0) goto failed;
                        Py_XDECREF(tmp);
                        tmp = NULL;
                } else {
                        PyErr_SetString(PyExc_TypeError, "required field \"c\" missing from Const");
                        return 1;
                }
                if (PyObject_HasAttrString(obj, "constant")) {
                        int res;
                        tmp = PyObject_GetAttrString(obj, "constant");
                        if (tmp == NULL) goto failed;
                        res = obj2ast_expr_const(tmp, &constant, arena);
                        if (res != 0) goto failed;
                        Py_XDECREF(tmp);
                        tmp = NULL;
                } else {
                        PyErr_SetString(PyExc_TypeError, "required field \"constant\" missing from Const");
                        return 1;
                }
                *out = Const(c, constant, lineno, col_offset, arena);
                if (*out == NULL) goto failed;
                return 0;
        }
...
int
obj2ast_expr_const(PyObject* obj, expr_const_ty* out, PyArena* arena)
{
        PyObject* tmp = NULL;

        if (PyObject_IsInstance(obj, (PyObject*)no_const_type)) {
                *out = no_const;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_1_type)) {
                *out = dummy_1;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)mutable_const_type)) {
                *out = mutable_const;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_2_type)) {
                *out = dummy_2;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_3_type)) {
                *out = dummy_3;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_4_type)) {
                *out = dummy_4;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)content_const_type)) {
                *out = content_const;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_5_type)) {
                *out = dummy_5;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_6_type)) {
                *out = dummy_6;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_7_type)) {
                *out = dummy_7;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_8_type)) {
                *out = dummy_8;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_9_type)) {
                *out = dummy_9;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_10_type)) {
                *out = dummy_10;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)dummy_11_type)) {
                *out = dummy_11;
                return 0;
        }
        if (PyObject_IsInstance(obj, (PyObject*)pure_const_type)) {
                *out = pure_const;
                return 0;
        }
        ...
        tmp = PyObject_Repr(obj);
        if (tmp == NULL) goto failed;
        PyErr_Format(PyExc_TypeError, "expected some sort of expr_const, but got %.400s", PyString_AS_STRING(tmp));
failed:
        Py_XDECREF(tmp);
        return 1;
}
...
init_ast(void)
{
        ...
        if (PyDict_SetItemString(d, "expr_const", (PyObject*)expr_const_type) <
            0) return;
        if (PyDict_SetItemString(d, "no_const", (PyObject*)no_const_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_1", (PyObject*)dummy_1_type) < 0)
            return;
        if (PyDict_SetItemString(d, "mutable_const",
            (PyObject*)mutable_const_type) < 0) return;
        if (PyDict_SetItemString(d, "dummy_2", (PyObject*)dummy_2_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_3", (PyObject*)dummy_3_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_4", (PyObject*)dummy_4_type) < 0)
            return;
        if (PyDict_SetItemString(d, "content_const",
            (PyObject*)content_const_type) < 0) return;
        if (PyDict_SetItemString(d, "dummy_5", (PyObject*)dummy_5_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_6", (PyObject*)dummy_6_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_7", (PyObject*)dummy_7_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_8", (PyObject*)dummy_8_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_9", (PyObject*)dummy_9_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_10", (PyObject*)dummy_10_type) < 0)
            return;
        if (PyDict_SetItemString(d, "dummy_11", (PyObject*)dummy_11_type) < 0)
            return;
        if (PyDict_SetItemString(d, "pure_const", (PyObject*)pure_const_type) <
            0) return;
